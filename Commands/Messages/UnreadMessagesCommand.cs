...
{
    [Command("unreadmsg", "View all unreaded in selected conversation.")]
    public class UnreadMessagesCommand : BaseMessagesCommand
    {
        public UnreadMessagesCommand(IServiceProvider serviceProvider) : base(serviceProvider)
        {
        }
        ...
        public override async Task OnMessage(Message message, CancellationToken cancellationToken)
        {
            ...
            {
                var messages = vkMessages.GetUnreadMessagesAsync(vkMessages.SelectedConversationId.Value);
